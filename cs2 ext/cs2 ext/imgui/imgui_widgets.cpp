...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const float rounding = square_sz * 0.35f; // Rounded corners

    ImDrawList* draw_list = window->DrawList;

    // Checkbox Background (Smooth Glow Effect)
    ImU32 bg_col = hovered ? IM_COL32(90, 0, 140, 220) : IM_COL32(50, 0, 90, 200);
    ImU32 border_col = IM_COL32(180, 0, 255, 255);
    draw_list->AddRectFilled(check_bb.Min, check_bb.Max, bg_col, rounding);
    draw_list->AddRect(check_bb.Min, check_bb.Max, border_col, 0, ImDrawFlags_RoundCornersAll, 2.0f);

    // Outer Glow Effect when Active
    if (*v)
    {
        draw_list->AddRect(check_bb.Min - ImVec2(2, 2), check_bb.Max + ImVec2(2, 2), IM_COL32(180, 0, 255, 180), 0, ImDrawFlags_RoundCornersAll, 2.0f);
        draw_list->AddRect(check_bb.Min - ImVec2(3, 3), check_bb.Max + ImVec2(3, 3), IM_COL32(180, 0, 255, 80), 0, ImDrawFlags_RoundCornersAll, 3.5f);
    }
    ...
    if (*v)
    {
        ...
        ImVec2 p1 = ImVec2(check_bb.Min.x + pad, check_bb.Min.y + square_sz * 0.6f);
        ImVec2 p2 = ImVec2(check_bb.Min.x + square_sz * 0.4f, check_bb.Min.y + square_sz - pad);
        ImVec2 p3 = ImVec2(check_bb.Max.x - pad, check_bb.Min.y + square_sz * 0.25f);
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    float slider_width = 150.0f; // Width of slider
    const float height = GetFrameHeight();

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ImVec2 pos = window->DC.CursorPos;
    ImRect total_bb(pos, pos + ImVec2(slider_width + label_size.x + 10, height));
    ...
    if (held)
    {
        float mouse_delta = GetIO().MouseDelta.x;
        float value_range = v_max - v_min;
        float sensitivity = value_range / slider_width;

        *v += mouse_delta * sensitivity;
        *v = ImClamp(*v, v_min, v_max);
        MarkItemEdited(id);
    }

    float progress = (*v - v_min) / (v_max - v_min);
    ImRect slider_bb(pos, pos + ImVec2(slider_width, height));

    ImDrawList* draw_list = window->DrawList;

    // Background Bar
    draw_list->AddRectFilled(slider_bb.Min, slider_bb.Max, IM_COL32(50, 0, 90, 200), 5.0f);

    // Filled Slider Bar (animated effect)
    float fill_width = progress * slider_width;
    draw_list->AddRectFilled(slider_bb.Min, ImVec2(slider_bb.Min.x + fill_width, slider_bb.Max.y), IM_COL32(180, 0, 255, 255), 5.0f);

    // Glowing Effect
    draw_list->AddRect(slider_bb.Min, slider_bb.Max, IM_COL32(180, 0, 255, 180), 5.0f, ImDrawFlags_RoundCornersAll, 1.5f);
    draw_list->AddRect(slider_bb.Min - ImVec2(1, 1), slider_bb.Max + ImVec2(1, 1), IM_COL32(180, 0, 255, 80), 5.0f, ImDrawFlags_RoundCornersAll, 2.5f);

    // Slider Handle (smooth rounded circle)
    ImVec2 handle_pos = ImVec2(slider_bb.Min.x + fill_width - 5, slider_bb.Min.y + height / 2);
    draw_list->AddCircleFilled(handle_pos, 7.0f, IM_COL32(255, 255, 255, 255));

    // Label Text
    RenderText(ImVec2(slider_bb.Max.x + 10, slider_bb.Min.y + height * 0.2f), label);

    return held; // Return true if actively dragging
}
